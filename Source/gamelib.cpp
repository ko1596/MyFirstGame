...
void CGameState::ShowInitProgress(int percent)
{
    ...
    CDDraw::BltBackColor(RGB(255, 255, 255));		// 將 Back Plain 塗上預設的顏色
    CMovingBitmap loading;						// 貼上loading圖示
    loading.LoadBitmap(IDB_LOADING);
    loading.SetTopLeft((SIZE_X - loading.Width()) / 2, y1 - loading.Height() - 25);
    loading.ShowBitmap();
    //
    // 以下為CDC的用法
    //
    ...
    CBrush* pb, b(RGB(133, 201, 100));				// 畫綠色 progress框
    pb = pDC->SelectObject(&b);
    pDC->Rectangle(x1, y1, x2, y2);
    CBrush b1(RGB(169, 239, 136));				// 畫黑色 progrss中心
    pDC->SelectObject(&b1);
    pDC->Rectangle(progress_x1, progress_y1, progress_x2_end, progress_y2);
    CBrush b2(RGB(255, 245, 118));					// 畫黃色 progrss進度
    pDC->SelectObject(&b2);
...
void CGame::OnDraw()
{
    ...
    if (!running)
    {
        ...
        bmp.SetTopLeft(0, (SIZE_Y - bmp.Height()) / 2 );
        bmp.ShowBitmap();
    }
